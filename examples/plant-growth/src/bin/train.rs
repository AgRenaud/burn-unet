...
use plant_growth::dataset::LaboroTomatoDataset;
...
pub fn main() -> Result<()> {
    ...
    let train_dataset = match LaboroTomatoDataset::new_from_folders(&train_dir) {
        Ok(dataset) => {
    ...
    let valid_dataset = match LaboroTomatoDataset::new_from_folders(&valid_dir) {
        Ok(dataset) => {
